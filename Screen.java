...
public class Screen extends JPanel implements MouseListener{
	...
	Screen(int screenWidth, int screenHeigth,float deltaTime, Player player[], Zombie zombie[], int id){
		...
		try {
			...
			font = Font.createFont(Font.TRUETYPE_FONT, new File("font.ttf")).deriveFont(Font.BOLD, 16);
			...
			bloodsplat = createResizedCopy(ImageIO.read(new File("img/zombie/bloodsplat.png")), 70, 70);
	...
	public void paintComponent(Graphics g){
		...
		for(int i = 0; i<2; i++){
			playerMoveIndex = counter <= 0 ? player[i].getNextSpriteIndex() : player[i].getSpriteIndex();
			...
			if(player[i].shooting && !player[i].idle){
				playerShootIndex = counter <= 0 ? player[i].getNextShootSpriteIndex() : player[i].getShootSpriteIndex();
		...
		if(zombie != null){
			for(int i = 0, n = zombie.length; i < n; i++){
				...
				zombieMoveIndex = counter <= 0 ? zombie[i].getNextSpriteIndex() : zombie[i].getSpriteIndex();
				...
				if(zombie[i].attacking){
					zombieAttackIndex = counter <= 0 ? zombie[i].getNextAttackSpriteIndex() : zombie[i].getAttackSpriteIndex();
				...
				if(zombie[i].hit){
					g.drawImage(bloodsplat, zombie[i].x, zombie[i].y, null);
					zombie[i].hit = false;
				}
			}
		}
